...
if (-not $env:ARGUS_START_SELF_UPDATED) {
  ...
  if ($skipSelfUpdate) {
    ...
    function Get-ArgusGitHubText([string]$RepoPath) {
      ...
      # Windows PowerShell's ``Set-Content -Encoding utf8`` adds its own BOM.
      # Strip a downloaded BOM first or the refreshed script starts with two
      # BOMs and PowerShell parses the first comment as an invalid command.
...
try {
  ...
  if (-not $allowStale -and $localBuild) {
    $localN = 0
    $remoteN = 0
    if ($localBuild -match 'v(\d+)\.(\d+)') { $localN = ([int]$Matches[1] * 10000) + [int]$Matches[2] }
    try {
      $fetchCode = Invoke-ArgusGit -Root $Root -GitArgs @("fetch", "origin")
      if ($fetchCode -eq 0) {
        $prev = $ErrorActionPreference
        $ErrorActionPreference = "Continue"
        try { $remoteText = git -C $Root show "origin/main:apps/eoc/src/lib/build.ts" 2>$null } finally { $ErrorActionPreference = $prev }
        $remoteBuild = Get-ArgusBuildIdFromText "$remoteText"
        if ($remoteBuild -match 'v(\d+)\.(\d+)') { $remoteN = ([int]$Matches[1] * 10000) + [int]$Matches[2] }
      }
    } catch { }
    if ($localN -gt 0 -and $remoteN -gt 0 -and $localN -gt $remoteN) {
      Write-Host ('Keeping local {0} - GitHub is only older. Start will not reset this PC.' -f $localBuild)
      $allowStale = $true
      $env:ARGUS_ALLOW_STALE = "1"
    }
  }
  if ($allowStale) {
  ...
  if ($apiReadyNow -and $eocReadyNow -and $workerReadyNow -and -not $updated) {
    ...
    try {
      ...
      if (-not (Test-ArgusWorkerFresh $Root)) {
        Write-Host "Worker topology incomplete - recycling main + Micro lanes..."
