...
function Clear-ArgusArqBacklog {
  ...
  try {
    ...
    foreach ($k in $keys) {
      if ($k) { docker exec argus-redis redis-cli DEL "$k" 2>$null | Out-Null }
    }
...
function Start-ArgusWorkerProcess([string]$Root) {
  ...
  try {
    ...
    if ($existing.Count -eq 1) {
      Write-Host ("OK  Worker already starting/running (PID {0})" -f $existing[0])
      return $existing[0]
    }
  ...
  try {
    ...
    Write-Host "Starting Argus worker (single process: health + market scans)..."
    ...
    Start-Sleep -Seconds 2
    $roots = @(Get-ArgusArqWorkerPids)
    if ($roots.Count -gt 1) {
      Write-Host ("WARN: {0} ARQ worker trees - keeping PID {1}, killing extras" -f $roots.Count, $proc.Id)
      foreach ($extraPid in $roots) {
        if ($extraPid -ne $proc.Id) {
          Stop-Process -Id $extraPid -Force -ErrorAction SilentlyContinue
        }
      }
    }
...
function Test-ArgusWorkerFresh([string]$Root) {
  # Prefer a single live health_supervisor ARQ process tree.
  $roots = @(Get-ArgusArqWorkerPids)
  if ($roots.Count -eq 1) { return $true }
  if ($roots.Count -gt 1) {
    Write-Host ("WARN: {0} ARQ worker trees - culling to one health_supervisor" -f $roots.Count)
    $keep = $null
    try {
      $hs = @(Get-ArgusArqWorkerProcesses | Where-Object {
        $_.CommandLine -like "*workers.health_supervisor.worker*"
      })
      $hsIds = @($hs | ForEach-Object { $_.ProcessId })
      $keep = @($hs | Where-Object { $hsIds -notcontains $_.ParentProcessId } |
        Select-Object -First 1 -ExpandProperty ProcessId)
    } catch { }
    if (-not $keep) { $keep = $roots[0] }
    foreach ($extraPid in $roots) {
      if ($extraPid -ne $keep) {
        Stop-Process -Id $extraPid -Force -ErrorAction SilentlyContinue
      }
