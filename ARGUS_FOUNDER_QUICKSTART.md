...
3. Open http://127.0.0.1:3000/argus-build.txt (or `/api/argus-build`) — must say **live-monitor-v2.51**
4. Hard-refresh Home (**Ctrl+F5**) — Build chip must match

If PowerShell says **cannot find file** / no Argus folder: the updater now **clones** `%USERPROFILE%\Desktop\Argus` (or OneDrive Desktop) from GitHub. Install [Git for Windows](https://git-scm.com/download/win) if `git` is missing, then re-run.

Reports may land on **OneDrive Desktop** or `%USERPROFILE%` — search for `Argus-update-report.txt` if you do not see it on Desktop.
